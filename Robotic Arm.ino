...
void setup() {
  ...
  motor1.setSpeed(100);//lift right --> FORWARD is arm going down
  motor2.setSpeed(100);//rotate --> BACKWARD is clk wise movement
  motor3.setSpeed(100);//left lift --> FORWARD is arm going down
  motor4.setSpeed(100);//hand  --> FORWARD is closing of hand
  motor1.run(RELEASE);
  motor2.run(RELEASE);
  motor3.run(RELEASE);
  motor4.run(RELEASE);
